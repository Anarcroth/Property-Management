...
// If the user has liked more articles, we have recommendations for him.
const numOfLikedArticlesThreshold = 9;

const translateCategoriesToLabels = function(categoryArray){
    const warIndex = categoryArray.indexOf('War');
    if(warIndex!=-1){
        categoryArray[warIndex] = 'International Conflicts';
    }
    return categoryArray;
};

const translateCategoryToLabel = function(category){
 return (category=='War' ? 'International Conflicts' : category);
};
